...
const WedThemeFormModal = ({ show, onHide, refresh, existing }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!existing) {
        await axios.post("http://localhost:5005/api/theme", formData);
      } else {
        // Update description
        await axios.put(`http://localhost:5005/api/theme/${existing._id}`, { description });

        // Add new images if any
        if (images.length > 0) {
          await axios.post(`http://localhost:5005/api/theme/images/${existing._id}`, formData);
        }
      }
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Theme Decoration</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          <Form.Group>
...
export default WedThemeFormModal;
