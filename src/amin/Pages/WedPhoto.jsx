...
import WedPhotoModal from "./WedPhotoModal";
...
const WedPhoto = () => {
  ...
  const fetchData = async () => {
    const res = await axios.get("http://localhost:5005/api/WedPhoto");
    setData(res.data);
  };
...
const deleteImage = async (imgName) => {
  ...
  return (
    <div className="container py-4">
      <h2 className="text-center mb-4">Wedding Photo  - Admin Panel</h2>
      ...
      <WedPhotoModal show={showModal} onHide={() => setShowModal(false)} refresh={fetchData} existing={data} />
    </div>
  );
};

export default WedPhoto;
