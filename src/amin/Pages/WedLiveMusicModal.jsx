...
const WedLiveMusicModal = ({ show, onHide, refresh, existing }) => {
  ...
  const deleteExistingImage = async (imgName) => {
    ...
    try {
      await axios.delete(`http://localhost:5005/api/WedLiveMusic/delete-image/${encodeURIComponent(imgName)}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!existing) {
        await axios.post("http://localhost:5005/api/WedLiveMusic", formData);
      } else {
        // Update description
        await axios.put(`http://localhost:5005/api/WedLiveMusic/${existing._id}`, { description });

        // Add new images if any
        if (images.length > 0) {
          await axios.post(`http://localhost:5005/api/WedLiveMusic/images/${existing._id}`, formData);
        }
      }
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Wedding Live Music</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          <Form.Group>
...
export default WedLiveMusicModal;
